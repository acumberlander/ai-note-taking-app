  ...
  try {
      const response = await openai.chat.completions.create({
          ...
          messages: [
              {
                  role: "system",
                  content: "You are a helpful assistant that generates 200 words or less for a request a user gives. They will want a note about the given topic. Could be a list or a paragraph or any suitable response."
              },
              {
                  role: "user",
...
export const trimCommand = async (
  ...
  const systemPrompt = `
Extract only the content of the note from the user's speech. Remove any command-like phrases. Be very exact with the remaining content, copying it word for word.
`;
